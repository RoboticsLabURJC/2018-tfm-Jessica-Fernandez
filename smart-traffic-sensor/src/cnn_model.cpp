...
std::string TF_PB_PATH = "../cfg/prueba.pb";
...
int CNNModel::find_blobs(colorspaces::Image& _frame, std::vector<Blob*>& new_blobs)
{
   ...
   std::cout << " Total size new blobs: " <<new_blobs.size()<<std::endl;
   for(int i = 0; i < detectionsCount && i < new_blobs.size(); ++i)
   {
      ...
      if (scores(i) > TF_PREDICTION_THRESSHOLD)
      {
         ...
         std::cout << "Detection " << (i+1) << ": class: " << boxClass << " cat:" << category << " " << label.str() << ", box: (" << x1 << "," << y1 << "), (" << x2 << "," << y2 << ")" <<"Blob counter"<< blob_counter<< std::endl;
