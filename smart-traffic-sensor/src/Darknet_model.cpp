...
int DarknetModel::find_blobs(colorspaces::Image& _frame, std::vector<Blob*>& new_blobs)
{
   ...
   cv::cvtColor(new_frame,rgbImage,CV_RGB2BGR);
    ...
    for (auto it = detections.data.begin(), end=detections.data.end(); it !=end; ++it){
		...
		curr_vehicle->set_klt_vehice(0);
		if (error == 0)
		{
			fs <<category<<" "<< it->detectionBox.x<< " "<< it->detectionBox.y<<" "<< it->detectionBox.width<<" "<< it->detectionBox.height<<" "<<it->probability<<endl;
			//std::cout<< it->detectionBox.x << " " << it->detectionBox.y << " " << it->detectionBox.height << " " << it->detectionBox.width << " category: "<<clase_vehicle<< std::endl;
			//std::cout<<"clase_vehicle: "<<clase_vehicle<<'\n';
			//putText(_frame,clase_vehicle, Point(x1,y1-2), FONT_HERSHEY_DUPLEX, 0.5, Scalar(255,0,0), 1);
			//putText(_frame,std::to_string(int(x1)), Point(x1,y1-2), FONT_HERSHEY_DUPLEX, 0.5, Scalar(255,0,0), 1);
			//putText(_frame,std::to_string(int(y1)), Point(x1,y1+13), FONT_HERSHEY_DUPLEX, 0.5, Scalar(255,0,0), 1);
 			//rectangle(_frame, Point(x1,y1), Point(x2,y2), Scalar(255,255,255), 1);
			error = 0;
		}
		//printf("curr_vehicle i = %d , x=%d , y=%d \n",blob_counter+1,curr_vehicle->get_2d_center().x, curr_vehicle->get_2d_center().y);
	blob_counter++;
    }
    fs.close();
