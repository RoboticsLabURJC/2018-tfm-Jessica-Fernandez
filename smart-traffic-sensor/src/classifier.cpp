 ...
 *
 */
void Classifier::update_vehicle(Vehicle* vehicle){
   ...
   camera->reproject(centro, &vehicle_3d_center, VehicleModel::get_category_dimension_x(vehicle->get_category())/2);

   memset(&projection, 0, sizeof(Tpolygon));
   project_model_and_shadow(vehicle->get_category(), vehicle_3d_center, &projection);

   vehicle->set_current_3d_center(vehicle_3d_center);
   vehicle->set_projection(&projection);
}
