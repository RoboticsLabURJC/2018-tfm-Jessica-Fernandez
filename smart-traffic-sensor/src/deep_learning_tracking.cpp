...
std::string DETECTION_FILE1 = "../detection1/";
std::string DETECTION_FILE2 = "../detection2/";
 ...
 */
                                   ...
                                   ,unsigned int num_blobs)
  {
   ...
   // Crea un fichero de salida
    ofstream fs(DETECTION_FILE1+std::to_string(contador_file_txt1)+".txt"); 
   ...
   for (unsigned int i=0; i<tracked_blobs.size(); i++)
   {
	 ...
	 Vehicle* curr_vehicle_txt = static_cast<Vehicle*> (blob);
		...
		blob->set_probability(1);
         fs <<curr_vehicle_txt->get_category()<<" "<< blob->get_rect().x<< " "<< blob->get_rect().y<<" "<< blob->get_rect().width<<" "<< blob->get_rect().height<<" "<<blob->get_probability()<<endl;
...
bool DeepLearningTracker::match_blob_using_distance_deep_learning(Blob* blob, vector<Blob*>& tmp_blobs, unsigned int num_blobs, Blob* matched_blob)
{
   ...
   for (unsigned int j=0; j<num_blobs; j++)
   {
      ...
      //LOG ("Distance matchinf is %f \n",inter_blob_2d_dis);
      //LOG ("blob matching x= %d , y=%d --- tmp x=%d , y=%d \n",blob->get_2d_center().x, blob->get_2d_center().y,tmp_blobs[j]->get_2d_center().x,tmp_blobs[j]->get_2d_center().y);
	//LOG("distance 2 d inter_blob_2d_dis = %d \n", inter_blob_2d_dis);
      //if (blob->IsBlobInProximityElipse(tmp_blobs[j]) && (inter_blob_2d_dis < distance))
      
      //if ((inter_blob_2d_dis < distance) && inter_blob_2d_dis < max_distance_vehicle )
      if (blob->IsBlobInProximityElipse(tmp_blobs[j]) && (inter_blob_2d_dis < distance))
      {
   ...
   {
      //LOG ("Distance matchinf is %f \n",distance);
      //LOG ("blob matching x= %d , y=%d --- tmp x=%d , y=%d \n",blob->get_2d_center().x, blob->get_2d_center().y,tmp_blobs[pos]->get_2d_center().x,tmp_blobs[pos]->get_2d_center().y);
      ...
      //LOG("Prueba isblobproximityelipse %d \n",prueba);
                                            ...
                                            unsigned int num_blobs)
{
   ...
   {
      //LOG("TMP BLOB X=%d , Y=%d \n",tmp_blobs[i]->get_2d_center().x, tmp_blobs[i]->get_2d_center().y);
      if (tmp_blobs[i]->already_matched())
      {
         ...
         if (tmp_blobs[i]->tracking_blobs_id.size() == 1)
         {
	       ...
	       if(tmp_blobs[i]->get_2d_center().y > size_image.height/2+size_image.height/5)
      ...
      //else if(road->in_road(tmp_blobs[i]->get_2d_center()) && tmp_blobs[i]->get_2d_center().y > size_image.height/2)
       else if(road->in_road(tmp_blobs[i]->get_2d_center()) && (tmp_blobs[i]->size() > 400) && 
		GET_LINE_POINT_DISTANCE(tmp_finish_line, tmp_blobs[i]->get_2d_center()) > MIN_DISTANCE && tmp_blobs[i]->get_2d_center().y > size_image.height/2)
      {
...
void DeepLearningTracker::track_blobs_klt(colorspaces::Image& new_frame
                                   ,timeval timestamp
                                   ,vector<Blob*>& tracked_blobs
                                   ,unsigned int num_blobs)
  {
   ...
   for (unsigned int i=0; i<tracked_blobs.size(); i++)
   {
      ...
      if (match_blob_using_klt_prueba(blob,  num_blobs, &matched_blob))
      {
         /** Check if we are close to the finish line, in this case calculate the blob
           *  trajectory related stats.
           */
...
bool DeepLearningTracker::match_blob_using_klt_prueba(Blob* blob, unsigned int num_blobs, Blob* matched_blob)
{
   int count = MAX_COUNT;
