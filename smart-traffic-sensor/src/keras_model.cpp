	...
	if(it->classId < 0)
 	{
		...
		if (error == 0)
		{
			//std::cout<< it->boundingBox.x << " " << it->boundingBox.y << " " << it->boundingBox.height << " " << it->boundingBox.width << " blob_counter"<<blob_counter<<" category: "<< clase_vehicle<<std::endl;
			//std::cout<<"clase_vehicle: "<<clase_vehicle<<'\n';
...
void KerasModel::free_memory_keras_model()
{
	pValue = PyObject_CallMethodObjArgs(pInstance, PyString_FromString("free_memory"), NULL);
	Py_DECREF(pValue);
}
