...
std::string TF_LABELLIST_PATH1 = "../src/label.names";
...
int KerasModel::find_blobs(colorspaces::Image& _frame, std::vector<Blob*>& new_blobs)
{
   ...
   //std::cout<<"pasado getkerasinferences"<<'\n';
   int blob_counter = 0;
   //Vehicle* curr_vehicle = static_cast<Vehicle*> (new_blobs[blob_counter]);
   //std::cout<<"classNamesFile: "<<classNamesFile<<'\n';
   ClassTypeGeneric typeConverter(classNamesFile);
   std::string clase_vehicle;
   //std::cout<<"vamos a por el for dichoso"<<'\n';
   //std::cout<<"detections.end() : "<<detections.end()<<'\n';
   //std::cout<<"detections.begin() : "<<detections.begin()<<'\n';
