...
{
   /*CNNModel* cnn_model = CNNModel::Instance();
   
   FeatureTracker* feature_tracker = FeatureTracker::Instance();

   /** Detect the new blobs in the new frame */
  /* unsigned int new_blobs_nb = cnn_model->find_blobs(_state.current_frame, _state.tmp_blobs);
   
   std::sort(_state.tmp_blobs.begin(), _state.tmp_blobs.end(), Blob::compare_size);
   
   feature_tracker->track_blobs(true,
                                _state.current_frame,
                                _state.timestamp,
                                _state.vehicles,
                                _state.tmp_blobs,
   ...
   return (new_blobs_nb > 0);*/
   this->tensorflow_model = TensorflowModel::Instance();
   ...
   /** Detect the new blobs in the new frame */
   unsigned int new_blobs_nb = this->tensorflow_model->find_blobs(_state.current_frame, _state.tmp_blobs);
...
bool TrafficMonitorAlgorithm::darknet_tracking()
{
  ...
  /* }else
   {
       new_blobs_nb = 0;
       deep_learning_tracker->track_blobs_klt(_state.current_frame,
                                _state.timestamp,
                                _state.vehicles,
                                 new_blobs_nb);
   }
   /* Ordenamos todos los blob detectador (por eso vamos de begin a new_blobs_nb) de menor a mayor tamaño.
      std::next(_state.tmp_blobs.begin(),new_blobs_nb) nos devuelve un puntero a la posición new_blobs_nb */
   //std::sort(_state.tmp_blobs.begin(), std::next(_state.tmp_blobs.begin(),new_blobs_nb), Blob::compare_size);
   /*for(i=0;i< new_blobs_nb;i++)
	{
		printf("blobbbb x=%d , y=%d \n",_state.tmp_blobs[i]->get_2d_center().x,_state.tmp_blobs[i]->get_2d_center().y);
	}*/
...
bool TrafficMonitorAlgorithm::iteration(const colorspaces::Image& currentImg, const timeval timeStamp)
{
   ...
   if (_cfg.play)
   {
      if (_cfg.trackingIsActive() || _cfg.classify)
      {
         ...
         {
            this->tensorflow_model->end_session();
	    //keras_tracking();
         }
	 else if (_cfg.darknetTrackingActive)
         {
	    /*KerasModel* keras_model = KerasModel::Instance();
	    keras_model->free_memory_keras_model();*/
            //free(this->tensorflow_model);
	    //delete(this->tensorflow_model);
	 ...
	 if ( cont == 100)
	     cont = 0;
	 /*cnnTrackingActive_prev = _cfg.cnnTrackingActive;
         kerasTrackingActive_prev = _cfg.kerasTrackingActive;
         darknetTrackingActive_prev = _cfg.darknetTrackingActive;*/	
      }
   }
