...
#define RIGHT_BUTTON 3
  ...
  ViewGtk::ViewGtk(Model& _model, const std::string& gladepath) throw ():
    ...
    //imagemenuitem5->signal_toggled().connect(sigc::mem_fun(this, &ViewGtk::onQuitToggled));
...
#define HEIGHT 128
      ...
      colorspaces::Image inputImage = model.getImage();

      cv::Rect rect_mat(0, 0, inputImage.height, inputImage.width);
      ...
      if (roi_inside_image)
      {
        ...
        cv::Mat croppedFaceImage = inputImage(rectifiedRoi).clone();
        colorspaces::Image outputImage (cv::Mat::zeros(HEIGHT,WITDH, CV_8UC3), colorspaces::ImageRGB8::FORMAT_RGB8);
        rectify(inputPoints, rectifiedPoints, affineMatrix);
        warpAffine(croppedFaceImage, outputImage, affineMatrix, size, INTER_LINEAR, BORDER_CONSTANT);
        drawImage(outputImage, drawingarea_rear.get());
      }
    }
  }
 ...
 */
  bool ViewGtk::onDrawingAreaMainExposeEvent(const Cairo::RefPtr<Cairo::Context>& ctx) {
    ...
    if (window)
    {
      ...
      if (cfg.classify || cfg.trackingIsActive())
      {
