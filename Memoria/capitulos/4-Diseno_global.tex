...
\begin{itemize}
    \item La base de datos recopilada por Redouane Kachach en su tesis doctoral, para la aplicación \textit{TrafficMonitor}, que es antecesor directo de este TFM~\cite{traffic_monitor_lab}.
    ...
    \item Imágenes recopiladas de cámaras en abierto de forma online durante este TFM.
\end{itemize} 
...
En la Figura se pueden ver algunas imágenes de nuestra base de datos.FIGURAAA!!
Hay que decir que todas las imágenes han sido etiquetadas a mano haciendo uso de la herramienta labelImg ~\cite{labelimg}, la cual permite guardar las etiquetas en archivos xml en formato PASCAL VOC (formato usado por ImageNet) o en formato YOLO en txt.

\section{Diseño de la Aplicación}
DIAGRAMA  un diagrama de entradas y salidas, como si fuera caja negra. Después un diagrama de bloques donde repasas cuales son los bloques
constituyentes de la aplicación y cómo se conectan entre sí!!!!!
...
El seguimiento se centra en la proximidad espacial, y si esta falla se utliza \acrshort{klt}. A todos los \textit{blob} detectados se les realizará un seguimiento a lo largo del tiempo. 
...
Las capas \textit{depthwise separable convolutions} siguen un proceso diferentea las convolucionales: 

\begin{enumerate}
    \item Realizan la operación \textit{depthwise convolution}, en la cual se aplican \textit{kernels} de igual profundidad que la imagen. Cada \textit{kernel} se emplea en cada canal de color por separado.
